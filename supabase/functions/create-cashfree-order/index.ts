...
serve(async (req)=>{
  ...
  try {
    ...
    if (!plan_id || typeof plan_id !== 'string') {
      return new Response(JSON.stringify({
        error: 'Missing or invalid plan_id'
      }), {
        status: 400,
        headers: {
          ...corsHeaders,
    ...
    if (pricing_id && !isValidUUID(pricing_id)) {
      return new Response(JSON.stringify({
        error: 'Invalid pricing_id format'
      }), {
        status: 400,
        headers: {
          ...corsHeaders,
    ...
    const { data: plan, error: planError } = await supabaseClient.from('subscription_plans').select('id, name, duration_months, is_active').eq('id', plan_id).eq('is_active', true).single();
    if (planError || !plan) {
      if (isDevelopment) {
    ...
    let pricing = null;
    try {
      let query = supabaseClient.from('plan_pricing').select('price, currency, country_code').eq('plan_id', plan_id).eq('is_active', true);
      if (pricing_id) {
    ...
    const apiUrl = CASHFREE_ENVIRONMENT === 'production' ? 'https://api.cashfree.com/pg/orders' : 'https://sandbox.cashfree.com/pg/orders';
